...
def detect(save_img=False):
    ...
    # create dict of colors from different games [jersey, number]
    game = opt.game
    color = ''
    color_dict = {'Hamilton' : ['blue', 'white'], 'Castleton' : ['white', 'blue']}
    ...
    # information about digits in jerseys
    #print('[INFO] jerseys grouped by common digits: ', count_dict)
    #print('[INFO] jerseys with unique digit: ', unique_digits)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    x1, y1, x2, y2 = int(xyxy[0])-20, int(xyxy[1])-20, int(xyxy[2]+20), int(xyxy[3]+20)
                    
                    # scale box by area
                    b_width, b_height = (x2 - x1), (y2 - y1)
                    area = b_width * b_height
                    exp = int((3500 - area) / 500)
                    box = [x1 + exp, y1 + exp, x2 - exp, y2 + exp]
                 
                    # color check and append to first lists
                    check = color_check(im0, box, color[0])
                    if check:
                        boxes.append(box)
                        confs.append(conf)
                ...
                for i in range(len(int_boxes)):
                    ...
                    if temp_box:
                        ...
                        # run number check
                        if temp_label == '':
                            num_check = number_check(im0, temp_box, color[1])
                            tess_numbers.append(num_check)
                            
                            # if number discovered
                            if num_check is not None:
                                temp_label = num_check
                                
                        # add box if non repeat label
                        if temp_label not in labels:
                            last_boxes.append(temp_box)
                            labels.append(temp_label)
                            confs.append(temp_conf)    

                print('tess numbers: ', tess_numbers) 
...
# helper function to check the jersey color of the player
def color_check(frame, box, color):
    ...
    # check jersey color for debugging mask
...
# helper function to try and read the jersey number using OCR
def number_check(frame, box, color):
    ...
    res_2 = selection.copy()
    
    # iterate through potential text
    for cnt in contours:
        x, y, w, h = cv2.boundingRect(cnt)
        rect = cv2.rectangle(res_2, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cropped = res_2[y:y+h, x:x+w]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--write', type=bool, required=False, help='write video')
