...
def delete_tracker(x, y):
    ...
    for key in trackers:
        print (trackers[key].bbox)
        if trackers[key].bbox[0] >= y >= trackers[key].bbox[0] + trackers[key].bbox[2]:
            if trackers[key].bbox[1] >= x >= trackers[key].bbox[1] + trackers[key].bbox[3]:
                print("Deleting tracker ", trackers[key])
        #if trackers[key].bbox:
            #break
