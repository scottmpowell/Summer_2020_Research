...
by Kevin Wu
    ...
    -and-keras/
Ultra Light Detector: 'Ultra Light Fast Generic Face Detector
1 MB' by Linzai
    URL: https://github.com/Linzaer/Ultra-Light-Fast-Generic-
    Face-Detector-1MB
...
def extract_face(image, boxes, required_size=(160,160)):
    ...
    if num_faces != 0:
        for i in range(num_faces):
            ...
            if (width > 10 or height > 10) and (width < height * 1.2):
...
if __name__ == "__main__":
    ...
    # set up ultra light detection variables [Linzai]
    input_img_size = 480
    define_img_size(input_img_size)
    
    # initialize ultra light detector, must import after
    # define img size [Linzai] 
    ...
    if net_type == 'slim':
        ...
        predictor_net = create_mb_tiny_fd_predictor(net, candidate_size=candidate_size, device=test_device)
    elif net_type == 'RFB':
        model_path = "models/pretrained/version-RFB-320.pth"
        net = create_Mb_Tiny_RFB_fd(len(class_names), is_test=True, device=test_device)
        predictor_net = create_Mb_Tiny_RFB_fd_predictor(net, candidate_size=candidate_size, device=test_device)
    else:
        print("[INFO] Detector not set")
        sys.exit(1)
    net.load(model_path)
    ...
    while True:
        ...
        # preprocess frame 
        process = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        
        # pass preprocessed frame for face detection [Linzai]
        boxes, labels, probs = predictor_net.predict(process, candidate_size / 2, threshold)
        num_faces = 0
        
        # reorganize boxes by removing fans detected above horizontal axis
        split_boxes = []
        for i in range(boxes.size(0)):
            split = boxes[i, :]
            if split[1] > 200:
                box = [split[0], split[1], split[2], split[3]]
                split_boxes.append(box)
        boxes = split_boxes.copy()
